...
# statsmodels.api permite realizar análisis estadísticos más detallados y estimación de modelos.
import statsmodels.api as sm
#Estas klibrerias nos ayudará a visualizar nuestros datos y resultados.
...
datos = pd.read_csv(r'C:\Users\HOLA\Downloads\sample_endi_model_10p.txt', sep=";")
...
# Filtrar los datos para incluir solo niños y niñas de la sierra que tengan valores válidos en la columna 'etnia'
...
poblacion_objetivo = datos[(datos["region"] == "Sierra") & (datos["etnia"] )]


#####################  EJERCICIO 1 #####################################################################

# Calcular cuántos niños se encuentran en la población objetivo
cantidad_niños = len(poblacion_objetivo)

# Calcular el conteo de la variable clave (quintil) respecto a esos niños
conteo_etnia = poblacion_objetivo['etnia'].value_counts()

print("Cantidad de niños en la población objetivo :", cantidad_niños)
print("Conteo de la variable 'etnia' respecto a esos niños:")
print(conteo_etnia)
...
datos_limpios['etnia_binario'] = datos_limpios['etnia'].apply(lambda x: 1 if x == 'Mestizo\Blanco' else 0)

# Filtrar los datos para incluir niños y niñas en la Sierra que tengan valores válidos en la columna 'etnia'
datos_sierra_etnia = datos_limpios[(datos_limpios['region'] == 'Sierra') & (datos_limpios['etnia_binario'] == 1)]
...
for i in variables:
    datos_sierra_etnia = datos_sierra_etnia[~datos_sierra_etnia[i].isna()]

# Agrupar los datos por region , etnia y contar el número de niños en cada grupo
conteo_por_etnia = datos_sierra_etnia.groupby(["region", "etnia"]).size()
print("Conteo de niños por categoría de 'etnia':")
print(conteo_por_etnia)
...
#R= El valor del parametro asoiciado a la varible n_hijos es negativo -21.3170 , por lo tanto
# eso significa que hay una correlacion negativa entre la varible n_hijos con la varibale de estudio etnia.

#######################  INTERPRETACION ##########################33

#De acuerdo a los datos propuestos se puede observar que el p valor es= 0.998 lo cual 
# al compararlo con el nivel de significancia de 5% es mayo por lo cual no e suna varible 
# estadisticamente significativo
# lo cual en la varibale etnia no esta definida por el numero de hijos. 


######################### RESPUESTAS #####################################################################

# 1. La precisión promedio del modelo aumenta cuando se utiliza el conjunto de datos filtrado en comparación con 
#el ejercicio anterior. Esto se debe a que el modelo ahora está utilizando un subconjunto más específico de datos 
#que están más relacionados con la variable objetivo.

# 2. La distribución de los coeficientes beta en comparación con el ejercicio anterior no necesariamente tiene 
#una tendencia clara. Dependerá de la naturaleza de los datos y de la relación entre las variables predictoras y 
#la variable objetivo. Es posible que algunos coeficientes beta aumenten o disminuyan en comparación con el ejercicio 
#anterior, pero no hay una regla general que se aplique a todos los coeficientes beta.
